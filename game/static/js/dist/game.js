...
class Player extends AcGameObject{
    ...
    shoot_fireball(tx, ty){
        ...
        let radius = this.playground.height * 0.008;
        let angle = Math.atan2(ty - this.y, tx - this.x);
        let vx = Math.cos(angle), vy = Math.sin(angle);
        let color = "orange";
        let speed = this.playground.height * 0.4;
        let move_length = this.playground.height * 1;
        new FireBall(this.playground, this, x, y, radius, vx, vy, color, speed, move_length, this.playground.height * 0.008);
    }
    ...
    update(){
        this.cooling_off_period += this.timedelta / 1000;
        if(!this.is_me && this.cooling_off_period > 4 && Math.random() < 1 / 240.0) {
    ...
    on_destroy(){
        for (let i = 0; i < this.playground.players.length; i ++) {
            if ( this.playground.players === this) {
...
class AcGamePlayground {
    ...
    show(){
        ...
        this.players = [];
        this.players.push(new Player(this, this.width / 2, this.height / 2, this.height * 0.04, "white", this.height * 0.1375, true));

        for (let i = 0; i < 5; i++) {
            this.players.push(new Player(this, this.width / 2, this.height / 2, this.height * 0.04, this.get_random_color(), this.height * 0.1375, false));
        }
